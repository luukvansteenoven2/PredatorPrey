...
def run_simulation(mountainOn, forestOn, visualize = True, seed=1):
    """
    ...
    if plotData:
        ...
        print(f"Average Lynx: {np.mean(LynxAmount)}")
        print(f"Average Hares: {np.mean(HaresAmount)}")
        
        peaks_populationLynx, _ = find_peaks(LynxAmount, distance=600, prominence=0.1)
        peaks_populationHares, _ = find_peaks(HaresAmount, distance=600, prominence=0.1)
    
        mean_Lynx = np.mean(LynxAmount)
        mean_Hares = np.mean(HaresAmount)
        
        fig, ax1 = plt.subplots()
        
        plt.axhline(y=mean_Hares, color='red', linestyle='--')

    
        color = 'red'
        ax1.set_xlabel('Time')
        ax1.set_ylabel('Hares Population', color=color)
        ax1.plot(time, HaresAmount, color=color)
        ax1.tick_params(axis='y', labelcolor=color)
        
        for peak in peaks_populationHares:
            ax1.axvline(x=time[peak], color=color, linestyle='--', linewidth=0.5)
            
        # A second axes that shares the same x-axis
        ax2 = ax1.twinx()  
        color = 'blue'
        ax2.set_ylabel('Lynx Population', color=color)
        ax2.plot(time, LynxAmount, color=color)
        ax2.tick_params(axis='y', labelcolor=color)
        
        for peak in peaks_populationLynx:
            ax2.axvline(x=time[peak], color=color, linestyle='--', linewidth=0.5)
            
        plt.axhline(y=mean_Lynx, color='blue', linestyle='--')
...
if __name__ == '__main__':
    ...
    visualize = True if input("Do you want to include live visualization in the model? (yes/no) ").lower() == 'yes' else False


    # Run the simulation
    run_simulation(mountainOn=mountainOn, forestOn=forestOn, visualize=visualize)